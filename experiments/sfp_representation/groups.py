"""Abstract finite group machinery over F_2 for Issue 009 (Fano / GL(3,2) / AGL(2,2) / K4).

Reusable, generalized version of the minimal inline checks that Gate 0 already
certified inside ``conformance.py``. This module is deliberately **self-contained
and abstract**: it knows nothing about Events, the SFP codec, ``sfp.py``, or
``topographo``. It works purely with labels:
...
A later task wires these groups to actual Events. Conventions (point labels,
``fano_lines``, ``gl_2_2`` row-tuple matrices, ``_xor2``, ``_apply``) are kept
bit-compatible with ``conformance.py`` so the two modules cannot contradict
each other.
...
# ---------------------------------------------------------------------------
# audit driver (template: experiments/mutual-frames/audit.py)
# ---------------------------------------------------------------------------
...
def audit() -> dict:
    payload = {
        ...
        "self_contained": "imports nothing beyond the standard library; does not "
                          "import sfp.py, conformance.py or topographo",
