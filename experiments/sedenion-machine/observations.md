...
The first deterministic basis search found `(x, y, z) = (e1, e2, e4)`, with `(x*y)*z = e7` and `x*(y*z) = -e7`.
A deterministic sparse search found `x = e1 + e10` and `y = e4 - e15`, with `x*y = 0`. Consequently `norm2(x*y) = 0` while `norm2(x)*norm2(y) = 4`.
