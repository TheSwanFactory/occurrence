...
**Remark 3.6a (the hypothesis is sufficient, not necessary — and 3.13's is not).** [T,C] Aut-invariance forces equilibrium, but far less does. On the basic crack, take the **sign pair** sharing a support, z± = (eᵢ ± e_{j+8})/√2 with i, j ∈ 1..7, i ≠ j. Then ½(M_{z₊} + M_{z₋}) = I **exactly** — a two-point measure, manifestly not Aut-invariant. *Proof.* z ↦ L_z is linear, so L_{z±} = L_a ± L_b with a = eᵢ/√2, b = e_{j+8}/√2, and the cross terms L_aᵀL_b + L_bᵀL_a cancel between the signs: M_{z₊} + M_{z₋} = 2(M_a + M_b). Each L_{e_k} is a signed permutation (the Cayley–Dickson structure constants are 0, ±1), hence orthogonal, so M_{e_k} = I, giving M_a = M_b = ½I. ∎ (Certified 8.9 × 10⁻¹⁶, worst of all 42 pairs; M_{e_k} = I at 0.0 [C].) **Two is minimal**: M_z has spectrum {0⁴, 1⁸, 2⁴}, so no single Kraus operator is I.
The shortcut stops there. Theorem 3.13 needs the full second moment, and the pair misses it (‖½Σzzᵀ − P_W/14‖ = 6.5 × 10⁻¹), so Φ_pair ≠ Φ₈₄ (4.3 × 10⁻¹) [C]. **Forced equilibrium is a first-moment condition on M and costs two points; reproducing Φ is a second-moment condition on z and costs the design.** The 84 discharges two separable obligations and only one of them is cheap — which sharpens 3.13's "only 16 × 16 moments, not the measure" by saying which of the theorem's consequences that economy does and does not buy. *Scope:* a statement about the basis-form crack; the argument leans on L_{e_k} being a signed permutation, which is a basis fact, not a property of general z ∈ Σ.
