...
**Definition 3.4 (crack).** Σ := {z ∈ 𝕊 : ‖z‖ = 1, ker L_z ≠ 0}. As a geometric object [T]: Σ is a single Aut(𝕊)-invariant orbit, equal to the homogeneous space G₂/SU(2) (where the SU(2) acts as the stabilizer of a base zero divisor). It is also a single G₂ orbit when viewed without the S₃ generation symmetry. Σ carries a **unique** Aut(𝕊)-invariant Borel probability measure μ (invariance forces block scalars on isotypics 1, 1′, 7⊗2; support on zero divisors and unit norm force the value). *Attribution.* Moreno's 2005 sequel states that the fixed-norm sedenion zero-divisor locus is the real Stiefel manifold V₇,₂, while the normalized singular set of annihilating pairs (z, w) with zw = 0 is homeomorphic to compact G₂; the projection to the first factor has fibre the unit sphere in the four-dimensional annihilator, S³ ≅ SU(2). Consequently Σ ≅ V₇,₂ ≅ G₂/SU(2) — up to the harmless √2 conversion between the two normalizations: Moreno normalizes each octonionic entry, we normalize the element, and multiplication by √2 is a radial homeomorphism between the loci. Moreno (1998) supplies the algebraic underpinning (special triples, Cor. 2.12, Thm. 2.13, the four-dimensional annihilator); its Introduction defers the pair-space topology to the sequel. The Stiefel model is developed in Biss–Dugger–Isaksen. For z ∈ Σ, M_z has spectrum {0⁴, 1⁸, 2⁴} and T_z = −P₀ + P₂ (BCDI, verified [C]).
...
> **REMARK 3.6a (equilibrium costs 2 points; the channel costs 14) — 2026-08-01:**
>
> [T,C] Aut-invariance forces equilibrium, but far less does. On the basic crack, take the **sign pair** sharing a support, z± = (eᵢ ± e_{j+8})/√2 with i, j ∈ 1..7, i ≠ j. Then ½(M_{z₊} + M_{z₋}) = I **exactly** — a two-point measure, manifestly not Aut-invariant. *Proof.* z ↦ L_z is linear, so L_{z±} = L_a ± L_b with a = eᵢ/√2, b = e_{j+8}/√2, and the cross terms L_aᵀL_b + L_bᵀL_a cancel between the signs: M_{z₊} + M_{z₋} = 2(M_a + M_b). Multiplication by a Cayley–Dickson basis unit maps the standard basis bijectively to a signed standard basis, so each L_{e_k} is orthogonal; hence M_{e_k} = I and M_a = M_b = ½I. ∎ (Certified 8.9 × 10⁻¹⁶, worst of all 42 pairs; M_{e_k} = I at 0.0 [C].) **Two is minimal**: M_z has spectrum {0⁴, 1⁸, 2⁴}, so no single Kraus operator is I.
>
> The shortcut stops there: the pair misses Theorem 3.13's full second moment (‖½Σzzᵀ − P_W/14‖ = 6.5 × 10⁻¹), so Φ_pair ≠ Φ₈₄ (4.3 × 10⁻¹) [C]. But the symmetric 84-point design is not minimal. Let σ be any derangement of {1,…,7}; the 14 points z_{i,±} = (eᵢ ± e_{σ(i)+8})/√2 have second moment P_W/14, because the signs cancel the cross terms and σ uses every coordinate once. Thus they reproduce Φ exactly by Theorem 3.13 [T,C]. Fourteen is minimal: the Choi matrix of Φ has rank 14, since its Kraus-vector span is {L_z : z ∈ W}, and z ↦ L_z is injective (L_z e₀ = z); any n-point Kraus representation has Choi rank at most n. **Forced equilibrium costs 2 points; reproducing Φ costs 14.** The 84 points provide symmetry, not support-minimality. *Scope:* the explicit constructions use the basis-form crack; the two lower bounds apply to all measures on Σ.

**Definition 3.7 (settlement channel).** Φ(X) := ∫_Σ L_zᵀ X L_z dμ(z). By 3.6, Φ is unital and trace-preserving; it is self-adjoint for the trace pairing and Aut-covariant. As a linear map on End(ℝ¹⁶) it is a 256 × 256 matrix. Note that Φ is trace-preserving *as a channel* (Tr Φ(X) = Tr X); the trace of Φ *as a 256 × 256 matrix* is 0, forced by the ± symmetry of the spectrum in 3.12 [C].
...
**Measurement 5.6 (stationary constants).** [M] The stationary (Furstenberg) measure of the oriented chain is tilted: spine share s\* = 0.13172 ± 0.00005 (three seeds; 84-design chain agrees), versus 1/8 for the uniform state. Quenched Lyapunov exponent λ_q = −0.01773 ± 0.00003; annealed exponent exactly 0 (by 3.6). Exact enrichment factorization [T backbone, M values]: s\* = E[A]·E[1/(1+τ)] + Cov = 0.1246 × 1.0794 − 0.0023 — the Hermitian-overlap mean alone *depletes* (0.1246 < 1/8); the Jensen "hull-protection" factor does all enriching; event-strain statistics anti-predict the tilt (τ-only reweighting gives 0.097). On the discrete crack, event resampling produces 1.0 × 10⁻³ rejected annihilating proposals per completed trajectory-step (N = 25,000, T = 120, single seed). A survival-conditioned ensemble instead has an annihilation hazard of ≈ 2.2 × 10⁻⁴ per at-risk trajectory-step, the difference reflecting survivorship selection; its stationary state statistics match the continuum [M].
...
### 7.1 What nonassociativity does and does not buy

A natural objection: 𝕊 is nonassociative, but every object in this paper — L_z, M_z, T_z, Φ — is an ordinary real matrix, composed by ordinary matrix multiplication. If the whole theory lives in End(ℝ¹⁶), where has the nonassociativity gone?

It has gone into the *choice of which matrices*. The map x ↦ L_x is not an algebra homomorphism (L_{xy} ≠ L_x L_y); that failure is exactly the nonassociativity. But the L_x themselves associate, being matrices, and Theorem 3.8(c) is emphatic about the consequence: **the associative envelope of {L_x : x pure} is all of End(ℝ¹⁶)** (dim 256, verified [C]), and 3.8(d) gives Lie closure 𝔰𝔬(16) (dim 120, verified [C]). No sedenionic structure survives inside the matrix algebra the L_x generate. The generated algebra is everything, hence structureless.

So nonassociativity is not a structure carried by the dynamics. It is a **selection principle**: it is what makes the set Σ of singular L_z nonempty (a composition algebra has no zero divisors, so ℝ, ℂ, ℍ, 𝕆 have empty crack), what makes Σ a single homogeneous orbit, and what fixes μ. Once the 84 Kraus operators are selected, the algebra has done its work and departs. Everything downstream is linear algebra on a distinguished finite family of matrices.

This is not a deflation. It is the correct statement of what is being claimed: not that nonassociative dynamics evade linear methods, but that a nonassociative algebra *nominates* a compact homogeneous family of singular operators whose averaged channel is exactly solvable, with a spectrum in sevenths. The nonassociativity is upstream of the dynamics, not inside it.

**Corollary for the physics reading.** The standard model's gauge algebra 𝔰𝔲(3) ⊕ 𝔰𝔲(2) ⊕ 𝔲(1) embeds in the imaginary octonions, a classical observation. Nothing in this paper reproduces, predicts, or constrains it. Theorem 5.2 proves Φ annihilates the generation-labeling observables exactly, so the S₃ "generation" structure carries no dynamical content in SSD; any generational asymmetry is initial-conditional. The names "time," "generation," and "survivorship" are tagged [I] throughout and are worth exactly zero. The paper's mathematical content stands or falls entirely on Sections 3–6.
...
1. P. Eakin, A. Sathaye, *[On automorphisms and derivations of Cayley–Dickson algebras](https://doi.org/10.1016/0021-8693(90)90221-9)*, J. Algebra 129 (1990) 263–278. ✓●
2. D. K. Biss, J. D. Christensen, D. Dugger, D. C. Isaksen, *Eigentheory of Cayley–Dickson algebras*, [arXiv:0905.2987](https://arxiv.org/abs/0905.2987); D. K. Biss, D. Dugger, D. C. Isaksen, *Large annihilators in Cayley–Dickson algebras*, Comm. Algebra (2008), [arXiv:math/0511691](https://arxiv.org/abs/math/0511691); *…II*, Bol. Soc. Mat. Mexicana, [arXiv:math/0702075](https://arxiv.org/abs/math/0702075). ✓● (spectra, sum rule, hulls, Stiefel model)
3. G. Moreno, *The zero divisors of the Cayley–Dickson algebras over the real numbers*, Bol. Soc. Mat. Mexicana (3) 4 (1998) 13–28; [arXiv:q-alg/9710013](https://arxiv.org/abs/q-alg/9710013). ● (the algebraic underpinning for Definition 3.4: special triples, Cor. 2.12, Thm. 2.13, four-dimensional annihilators; the pair-space topology is deferred to the sequel below.) G. Moreno, *Constructing zero divisors in the higher dimensional Cayley-Dickson algebras*, [arXiv:math/0512517](https://arxiv.org/abs/math/0512517) (2005). ● (fixed-norm single zero-divisor locus ≅ V₇,₂; normalized annihilating-pair set ≅ G₂; fibration with fibre S³ — the topological attribution for Definition 3.4)
4. S. Zhilina, zero-divisor graphs of Cayley–Dickson algebras (double-hexagon components); see A. E. Guterman, S. A. Zhilina, *[Relation graphs of the sedenion algebra](https://doi.org/10.1007/s10958-021-05367-6)*, J. Math. Sci. (2021). ✓ (component structure)
5. K.-C. Chan, D. Ž. Đoković, *[Conjugacy classes of subalgebras of the real sedenions](https://doi.org/10.4153/CMB-2006-048-6)*, Canad. Math. Bull. 49 (2006) 492–507. ● (prior triad count; our derivation independent)
6. R. B. Brown, *[On generalized Cayley–Dickson algebras](https://projecteuclid.org/euclid.pjm/1102992693)*, Pacific J. Math. 20 (1967) 415–422. ✓● (the ζ-generator; verified equal to exp((2π/3)K\*), Theorem 3.11)
7. R. D. Schafer, *An Introduction to Nonassociative Algebras*, Academic Press, 1966. ✓ (Moufang gate; derivation extension)
8. H. Furstenberg, *[Noncommuting random products](https://doi.org/10.1090/S0002-9947-1963-0163345-0)*, Trans. AMS 108 (1963) 377–428; P. Bougerol, J. Lacroix, *Products of Random Matrices with Applications to Schrödinger Operators*, Birkhäuser, 1985.
