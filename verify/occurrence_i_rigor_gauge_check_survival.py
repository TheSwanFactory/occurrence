...
def run_survival(retained_on_left, N, T, burn_in, seed, death_tol=1e-9):
    ...
    for t in range(T):
        ...
        if t >= burn_in and alive.sum() > 0:
            shares.append(spine_share_batch(X)[alive])
    n_alive_final = alive.sum()
    total_step_death_rate = sum(deaths_per_step) / (N * T)
    flat = np.concatenate(shares) if shares else np.array([])
    return flat.mean(), flat.std() / np.sqrt(len(flat)), n_alive_final, total_step_death_rate


if __name__ == "__main__":
    print("[Test D] Original Test-14-D3 conditions (N=1000, T=30, no burn-in), 5 seeds each orientation:")
    for seed in range(1, 6):
        m_std, se_std, na_std, dr_std = run_survival(False, N=1000, T=30, burn_in=0, seed=seed)
        m_swp, se_swp, na_swp, dr_swp = run_survival(True, N=1000, T=30, burn_in=0, seed=seed + 50)
        diff = m_std - m_swp
    ...
    for seed in range(1, 4):
        m_std, se_std, na_std, dr_std = run_survival(False, N=8000, T=400, burn_in=100, seed=seed)
        m_swp, se_swp, na_swp, dr_swp = run_survival(True, N=8000, T=400, burn_in=100, seed=seed + 100)
        diff = m_std - m_swp
        cse = np.sqrt(se_std ** 2 + se_swp ** 2)
